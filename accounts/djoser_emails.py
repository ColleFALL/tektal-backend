...
class ActivationEmail(email.ActivationEmail):
    def send(self, to, *args, **kwargs):
        ...
        link = f"https://active-tektal.vercel.app/activate.html?uid={uid}&token={token}"
...
class PasswordResetEmail(email.PasswordResetEmail):
    def send(self, to, *args, **kwargs):
        ...
        link = f"https://active-tektal.vercel.app/reset-password.html?uid={uid}&token={token}"

        html = f"""
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
          <h2>Réinitialisation de mot de passe</h2>
          <p>Cliquez sur le lien ci-dessous :</p>
          <p><a href="{link}">Continuer</a></p>
          <p>Si le bouton ne fonctionne pas, copiez-collez ce lien :</p>
          <p><a href="{link}">{link}</a></p>
        </div>
        """
