...
class PathListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            # Admin voit tout SAUF les deleted
            queryset = Path.objects.exclude(status='deleted').order_by('-created_at')
        elif hasattr(user, 'etablissement'):
            # Établissement voit ses chemins (sauf deleted) + les publiés
            queryset = Path.objects.filter(
                models.Q(user=user, status__in=['draft', 'published', 'hidden']) |
                models.Q(status='published')
            ).exclude(status='deleted').order_by('-created_at')
        else:
            # Utilisateur mobile : uniquement les publiés
...
class PathDetailView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            # Admin voit tout sauf deleted
            return Path.objects.exclude(status='deleted')
        elif hasattr(user, 'etablissement'):
            # Établissement voit ses chemins (sauf deleted) + les publiés
